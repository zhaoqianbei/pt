...
function get_topic_image($status= "read"){
	...
	switch($status){
		case "read": {
			return "<i class=\"unlocked icon pt-kaisuo fc8\" alt=\"read\" title=\"".$lang_forums['title_read']."\"></i>";
			break;
			}
		case "unread": {
			return "<i class=\"unlockednew icon pt-kaisuo\" alt=\"unread\" title=\"".$lang_forums['title_unread']."\"></i>";
			break;
		}
		case "locked": {
			return "<i class=\"locked icon pt-suo fc8\" src=\"pic/trans.gif\" alt=\"locked\" title=\"".$lang_forums['title_locked']."\"></i>";
			break;
		}
		case "lockednew": {
			return "<i class=\"lockednew icon pt-suo\" src=\"pic/trans.gif\" alt=\"lockednew\" title=\"".$lang_forums['title_locked_new']."\"></i>";
			break;
		}
	}
}
...
if ($action == "viewunread")
{
	...
	{
		print("</table>\n");
		print("<table border=\"0\" class=\"main\" cellspacing=\"0\" cellpadding=\"5\" width=\"\"><tr><td class=\"embedded\"><form method=\"get\" action=\"?\"><input type=\"hidden\" name=\"catchup\" value=\"1\" /><input type=\"submit\" value=\"".$lang_forums['text_catch_up']."\" class=\"btn\" /></form></td>");
		if ($n > $maxresults){
...
foreach ($overforums as $a)
{
	...
	print("<tr><td align=\"left\" class=\"colhead\" width=\"70%\">".htmlspecialchars($overforumname)."</td><td align=\"center\" class=\"colhead\">".$lang_forums['col_topics']."</td>" .
	...
	foreach ($forums as $forums_arr)
	{
		...
		"<br />".$forumdescription."</td></tr></table></td><td class=\"rowfollow\" align=\"center\" width=\"\">".$topiccount."</td><td class=\"rowfollow\" align=\"center\" width=\"\">".$postcount."</td>" .
