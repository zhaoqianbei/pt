...
function menu($selected = "home")
{
    ...
    print("<li><a href=\"https://www.zhaoqianbei.com\">" . $lang_functions['text_zqb'] . "</a></li>");
    print("</ul></div>");
...
<link rel="stylesheet" href="//at.alicdn.com/t/font_1856866_24ui53q95og.css" type="text/css" />
...
$caticonrow = get_category_icon_row($CURUSER['caticon']);
    ...
    while ($row = mysql_fetch_assoc($res)) {
        ...
        if ($count_dispname > $max_length_of_torrent_name) {
            $dispname = mb_substr($dispname, 0, $max_length_of_torrent_name - 23, "UTF-8") . "..";
        }
        ...
        if ($CURUSER['showcomnum'] != 'no') {
            ...
            if (!$row["comments"]) {
                ...
                print("<b><a href=\"details.php?id=" . $id . "&amp;hit=1&amp;cmtpage=1#startcomments\" >" . ($hasnewcom ? "<font class='new'>" : "") . $row["comments"] . ($hasnewcom ? "</font>" : "") . "</a></b>");
            }
...
{
    print("<textarea name='" . $taname . "' cols=\"100\" rows=\"8\" style=\"width: 1000px\" onkeydown=\"ctrlenter(event,'compose','qr')\"></textarea>");
...
function get_torrent_promotion_append($promotion = 1, $forcemode = "", $showtimeleft = false, $added = "", $promotionTimeType = 0, $promotionUntil = '')
{
    ...
    if (get_global_sp_state() == 1) {
        ...
        if ($promotion != 1) {
            $timeout = $lang_functions['text_will_end_in'] . $timeout;
        }
