...
elseif ($row['banned'] == 'yes' && get_user_class() < $seebanned_class)
	...
	if (!isset($_GET["cmtpage"])) {
		...
		if ($smalldescription_main == 'yes'){
			$fubiao = htmlspecialchars(trim($row["small_descr"]));
		}else{
			$fubiao = '无';
		}
		// PT下载方式
		...
		$sh = '<div class="social-share tal w18">分享给想分享的人：</div>';
		print($sh);
	...
	if ($imdb_id && $showextinfo['imdb'] == 'yes' && $CURUSER['showimdb'] != 'no')
	{
		...
		if ($bwrow['upname'] && $bwrow['downname'])
			tr($lang_details['row_uploader_bandwidth'], "<i class=\"speed_down icon pt-xiazai fcg\" alt=\"Downstream Rate\"></i> ".$bwrow['downname']."&nbsp;&nbsp;&nbsp;&nbsp;<i class=\"speed_up icon pt-shangchuan fcr\" alt=\"Upstream Rate\"></i> ".$bwrow['upname']."&nbsp;&nbsp;&nbsp;&nbsp;".$bwrow['ispname'],1);
...
print ("<table width =\"90%\"><tr><td class=\"text\" align=\"left\"><b>".$lang_details['text_quick_comment']."</b><form id=\"compose\" name=\"comment\" method=\"post\" action=\"".htmlspecialchars("comment.php?action=add&type=torrent")."\" onsubmit=\"return postvalid(this);\"><input type=\"hidden\" name=\"pid\" value=\"".$id."\" />");
