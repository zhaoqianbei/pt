...
if (!isset($CURUSER) || ($CURUSER["id"] != $row["owner"] && get_user_class() < $torrentmanage_class)) {
	...
	if (get_user_class()>= $torrentsticky_class || (get_user_class() >= $torrentmanage_class && $CURUSER["picker"] == 'yes')){
		...
		if(get_user_class()>=$torrentsticky_class)
		{
			...
			$pickcontent .= "<b>".$lang_edit['row_torrent_position'].":&nbsp;</b>".'<select name="sel_posstate" style="width: 100px;" id="pos_group">' .
			"<option" . (($row["pos_group"] == 0) ? ' selected="selected"' : '' ) . " value=\"0\">".$lang_edit['select_normal']."</option>" .
			"<option" . (($row["pos_group"] > 1) ? ' selected="selected"' : '' ) . ' value="2">'.$lang_functions['title_top_sticky']."</option>" .
			"<option" . (($row["pos_group"] == 1) ? ' selected="selected"' : '' ) . " value=\"1\">".$lang_edit['select_sticky']."</option>" .
			'</select>&nbsp;&nbsp;&nbsp;';
			
			$pickcontent .= '<select name="pos_until_select" id="pos_until_select">'.
			sprintf('<option value="-1"%s>%s</option>', EchoSelected($row['pos_until'] === '0000-00-00 00:00:00'), $lang_edit['select_forever']).
			sprintf('<option value="0"%s>%s</option>', EchoSelected($row['pos_until'] != '0000-00-00 00:00:00'), $lang_edit['select_until'])
			.sprintf('</select>&nbsp;<input type="text" name="pos_until" id="pos_until" value="%s" />', htmlspecialchars($row['pos_until']));
		}
