...
if ($action){
	...
	switch ($action) {
		case "personal":
			...
			usercpmenu ("personal");
			print ("<table border=0 cellspacing=0 cellpadding=5 width=90%>");
			if ($type == 'saved')
...
tr($lang_usercp['row_school'], "<select name=school>$schools</select>", 1);
			...
			tr_small($lang_usercp['row_avatar_url'], "<img width=\"80px\" src=".($CURUSER["avatar"] ? "'$CURUSER[avatar]'" : "'" . get_protocol_prefix() . $BASEURL . "/pic/default_avatar.png'")." name='avatarimg'><br />
...
<?php
print("<table border=0 cellspacing=0 cellpadding=3 width=90%><tr>".
"<td class=colhead align=left width=70%>".$lang_usercp['col_topic_title']."</td>".
